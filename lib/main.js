...
IronMQ.Queue = function(project, name, size) {
  ...
  var base = {
    ...
    message: function(id, cb) {
      ...
      if (_messages.length) {
        ...
        _messages.forEach(function(msg) {
          if (msg.id == id) {
            message = IronMQ.Message(_self, msg);
          }
        });
        if (message) return cb(null, message);
      }
      
      this.messages(function(err, messages) {
        if (err) return cb(err);
    ...
    deleteMessage: function(id, cb) {
      IronMQ._transport.del('/projects/'+this.project.id()+'/queues/'+_name+'/messages/'+id, null, function(err, response) {
        if (err) {
          if (cb) cb(err);
          return;
        }
        if (response.msg && response.msg == "Deleted") {
          if (cb) cb(null, response.msg);
        } else {
          if (cb) cb(response.msg);
        }
      });
...
IronMQ.Message = function(queue, data) {
  ...
  var base = {
    ...
    delete: function(cb) {
      this.queue.deleteMessage(_id, cb);
    },
    toJSON: function() {
      var obj = {
        id: _id,
